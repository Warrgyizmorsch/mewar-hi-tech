...
const LATEST_PROJECTS_DATA: ProjectItem[] = [
  {
    id: 1,
    src: "/images/latest-projects-1.jpg",
    title: "300 TPH Complete Jaw & Cone Plant",
    location: "Rajasthan, India",
    capacity: "300 TPH",
    tag: "Stationary Plant",
  },
  {
    id: 2,
    src: "/images/latest-projects-2.jpg",
    title: "Heavy Duty Track Mounted Crusher",
  ...
  {
    id: 3,
    src: "/images/latest-projects-3.jpg",
    title: "Aggregate Screening & Sizing Line",
  ...
  {
    id: 4,
    src: "/images/latest-projects-4.jpg",
    title: "High Performance VSI Sand Maker",
  ...
  {
    id: 5,
    src: "/images/latest-projects-5.jpg",
    title: "Wheel Mounted Mobile Crusher Unit",
  ...
  {
    id: 6,
    src: "/images/latest-projects-6.jpg",
    title: "Vibro Feeder & Hopper System",
  ...
  {
    id: 7,
    src: "/images/latest-projects-7.jpg",
    title: "Turnkey Quarry Processing Line",
  ...
  {
    id: 8,
    src: "/images/latest-projects-8.jpg",
    title: "Track Mounted Impactor Plant",
...
export default function LatestProjects() {
  ...
  const handlePrev = (e?: React.MouseEvent) => {
    ...
    if (lightboxIndex !== null) {
      setLightboxIndex((prev) =>
        prev === 0 ? LATEST_PROJECTS_DATA.length - 1 : prev! - 1
      );
    }
  };
  ...
  const handleNext = (e?: React.MouseEvent) => {
    ...
    if (lightboxIndex !== null) {
      setLightboxIndex((prev) =>
        prev === LATEST_PROJECTS_DATA.length - 1 ? 0 : prev! + 1
      );
    }
  };
  ...
  const featuredProject = LATEST_PROJECTS_DATA[0];
    ...
    <section className="section-padding-sm bg-background border-b border-border/60 relative overflow-hidden select-none">
      ...
      <Container className="relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
          ...
          {/* Main Hero Featured Project Card (Spans 7 cols on LG) */}
          <motion.div
          ...
          >
            ...
            <div className="relative z-20 p-6 sm:p-8 space-y-2 text-left">
              ...
              <div className="flex items-center gap-2 text-white/80 text-xs font-semibold pt-2">
                <span>Click to view full installation gallery</span>
                <ChevronRight size={14} className="text-primary group-hover:translate-x-1 transition-transform" />
              </div>
            </div>
          </motion.div>

          {/* Secondary Grid Column (Spans 5 cols on LG) */}
          <div className="lg:col-span-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-6">
            {remainingProjects.slice(0, 3).map((item, idx) => (
              <motion.div
                key={item.id}
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-6">
          {remainingProjects.slice(3).map((item, idx) => (
            <motion.div
              key={item.id}
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
            ...
            <div className="absolute top-6 left-6 right-6 flex justify-between items-center text-white z-20">
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm font-semibold text-white/80">
                  {lightboxIndex + 1} of {LATEST_PROJECTS_DATA.length}
                </span>
              </div>
              <button
                onClick={() => setLightboxIndex(null)}
              ...
              <div className="absolute bottom-0 inset-x-0 p-6 bg-gradient-to-t from-black/90 via-black/70 to-transparent text-left text-white flex flex-col sm:flex-row justify-between items-start sm:items-end gap-3">
                <div>
                  <div className="flex items-center gap-2 text-primary text-xs font-bold mb-1">
                    <MapPin size={14} />
                    <span>{LATEST_PROJECTS_DATA[lightboxIndex].location}</span>
                  </div>
                  <h3 className="text-xl font-bold uppercase tracking-wide">
                    {LATEST_PROJECTS_DATA[lightboxIndex].title}
                  </h3>
                </div>
                <div className="px-4 py-2 rounded-xl bg-white/10 backdrop-blur-md border border-white/10 text-xs font-bold">
                  Capacity: {LATEST_PROJECTS_DATA[lightboxIndex].capacity}
                </div>
              </div>
            </div>
